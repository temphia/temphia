    ...
    {#key $params["dtable"] && $params["layout"]}
      <TableUI
        ...
        on:on_change_to_card={(ev) =>
          app.nav.data_table_render_page(source, group, $params["dtable"], "/card")}
        on:on_table_change={(ev) =>
          app.nav.data_table_render_page(source, group, ev.detail)}
        on:on_change_to_grid={() =>
          app.nav.data_table_render_page(source, group, $params["dtable"])}
        on:admin_data_table={() =>
