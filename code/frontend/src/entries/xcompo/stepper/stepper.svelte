...
<div class="stepper {classesBase}" data-testid="stepper">
  ...
  {#if $state.total}
    <header
      class="stepper-header mt-5 {classesHeader}"
      transition:fade|local={{ duration: 100 }}
    >
      {#each Array.from(Array($state.total).keys()) as step}
        <div
          class="stepper-header-step -mt-5 {classesHeaderStep}"
          class:flex-1={isActive(step)}
        >
          <span class="badge bg-gray-700 rounded-full text-white px-2 py-1  {classesBadge(step)}"
