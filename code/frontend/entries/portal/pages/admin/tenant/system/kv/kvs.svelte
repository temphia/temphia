<script lang="ts">
  ...
  const load = async () => {
    loading = true;
    const resp = await api.list_system_kv({
      etype,
      last,
      prefix,
    });
...
{:else}
  <div class="p-4">
    <table class="min-w-full shadow rounded-lg overflow-auto">
      ...
      <tbody>
        {#each datas as data}
          ...
          >
            ...
            <td
              class="text-sm text-gray-900 font-light px-2 py-2 whitespace-nowrap max-w-md overflow-clip"
            >
              {data["value"] || ""}
            </td>
            <td
