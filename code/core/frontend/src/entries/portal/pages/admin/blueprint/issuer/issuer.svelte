<script lang="ts">
  ...
  (async () => {
    const manager = app.get_apm();
    const bapi = await manager.get_plug_api();
...
{#if all_plugs || selected_plugs.length != 0}
  <div class="flex justify-end">
    <button
      ...
      on:click={async () => {
        const manager = app.get_apm();
        const bapi = await manager.get_bprint_api();
        ...
        app.simple_modal_open(IssuerResult, {
          ticket: resp.data,
          close: app.simple_modal_close,
        });
      }}
    >
