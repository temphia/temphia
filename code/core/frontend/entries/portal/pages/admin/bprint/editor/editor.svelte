<script lang="ts">
  ...
  const add = () => {
    service.utils.small_modal_open(PanelAddFile, { beditor });
  };
  const save = () => {
    console.log(_mode);
    ...
    switch (_mode) {
      case MODE_FILE:
        service.utils.small_modal_open(PanelSaveFile, {
        ...
        break;
      default:
        service.utils.small_modal_open(PanelSave, {});
    }
  };

  const check = () => {
    service.utils.small_modal_open(PanelCheckFile, {
      filename: _current_file,
      beditor,
      finalSave: async () => {},
    });
