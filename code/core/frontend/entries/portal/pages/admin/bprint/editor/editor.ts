...
import type { AdminBprintAPI } from "../../../../../../lib/apiv2/admin";
...
export class Editor {
  ...
  constructor(bid: string, bapi: AdminBprintAPI) {
    this.bid = bid;
    this.bapi = bapi;
  ...
  load = async () => {
    const resp = await this.bapi.get(this.bid);
    if (resp.status !== 200) {
  ...
  save = async () => {
    const resp = await this.bapi.update(this.bid, { ...this.data });
    if (resp.status !== 200) {
      // fixme =>
      return;
    }
  ...
  uploadNewFile = (file: string, data: any) => {
    return this.bapi.add_file(this.bid, file, data);
  };
  ...
  saveFile = async (filename: string) => {
    ...
    const resp = await this.bapi.update_file(this.bid, filename, file);
    if (resp.status !== 200) {
  ...
  getFile = async (filename: string) => {
    ...
    const resp = await this.bapi.get_file(this.bid, filename);
    if (resp.status !== 200) {
      return;
    }
