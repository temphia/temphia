<script lang="ts">
  import { getContext } from "svelte";
  import type { PortalApp } from "../../../../app";
  import Layout from "../../layout.svelte";
  import { AutoTable, FloatingAdd } from "../../../../../../shared";

  export let aid: string;
  export let pid: string;
  let links = [];

  const app: PortalApp = getContext("__app__");

  const load = async () => {
    const papi = await app.get_apm().get_plug_api();
    const resp = await papi.agent_link_list(pid, aid);
    if (resp.status !== 200) {
      console.log("@err", resp);
      return;
    }
    links = resp.data;
  };

  load();
</script>

<Layout>
  <AutoTable
    action_key="id"
    actions={[
      {
        Name: "Edit",
        Action: (id) => {
          app.navigator.goto_admin_agents_link_edit(pid, aid, id);
        },
        drop: true,
        icon: "trash",
      },
      {
        Name: "Delete",
        Action: async (id) => {
          const papi = await app.get_apm().get_plug_api();
          await papi.agent_link_del(pid, aid, id);
          load();
        },
        drop: true,
        icon: "trash",
      },
    ]}
    key_names={[
      ["id", "ID"],
      ["name", "Name"],
      ["from_plug_id", "From Plug"],
      ["from_agent_id", "From Agent"],
      ["to_plug_id", "To Plug"],
      ["to_agent_id", "To Agent"],
    ]}
    datas={links}
  />
</Layout>

<FloatingAdd
  onClick={() => app.navigator.goto_admin_agents_link_new(pid, aid)}
/>
